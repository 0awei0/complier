...
class GenerateCode:
    ...
    def __init__(self, ast, end):
        ...
        self.code_block = self.first_view(CodeBlocks())
    ...
    # 检查重复定义变量
    def redeclared_variables(self, k, code_block, i):
        k = k.split(':')
        if k[0] == 'identifier':
            ...
            if i[0] in ['int', 'char', 'double', 'float', 'long']:
                if '=' in i:
                    ...
                    for k in i[1:temp_index]:
                        self.redeclared_variables(k, code_block, i)
                        # k = k.split(':')
                        # if k[0] == 'identifier':
                        #     for j in code_block.variable_list:
                        #         if k[0] == j:
    ...
    def second_view(self, block):
        while len(block.lines) != 0:
            i = block.lines[0]
            if isinstance(i, CodeBlocks):
                self.second_view(i)
                del block.lines[0]
            else:
                temp = i[0].split(':')
                if i[0] in ['int', 'char', 'double', 'float', 'long', 'def']:
                    if '=' not in i:
                        del block.lines[0]
                    else:
                        temp_index = i.index('=')
                    ...
                    if len(block.lines) > 0:
                        ...
                        if block.lines[0] == 'else' or block.lines[0][0] == 'else':
                            ...
                            if isinstance(block.lines[0], CodeBlocks):
                    ...
                    self.process_op(
                        block.lines[0][2].split(':')[1], block.lines[0][4].split(':')[1], block.lines[0][3],
                        block.lines[0][0].split(':')[1])
                    temp1 = self.code.pop()
                    temp2 = self.code.pop()
                    del block.lines[0]
                    self.second_view(block.lines[0])
    ...
    @staticmethod
    def gen_code(op, arg1, arg2, result):
...
def alg_test(filename="../Tests/test1.txt"):
    ...
    a = GenerateCode(ll1.ast.root, ll1.end)
    ...
    if len(a.warn) != 0:
        ...
        for i in range(len(a.code) - 1):
            print(a.code[i][0], a.code[i][1], a.code[i][2], )

            if isinstance(a.code[i][3], int):
                print(str(int(a.code[i][3]) + 1))
            else:
                print(a.code[i][3])
    print("*" * 20)
    print(a.code)
